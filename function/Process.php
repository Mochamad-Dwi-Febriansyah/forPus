...
if ($_GET['aksi'] == "masuk") {
    ...
    if ($cek > 0) {
        ...
        if ($row['role'] == "Admin") {
            ...
            $status = "Active now";
            $sql2 = mysqli_query($koneksi, "UPDATE user SET status = '{$status}' WHERE unique_id = {$row['unique_id']}");
    ...
    $query = mysqli_query($koneksi, "SELECT max(unique_id) as kodeTerakhir FROM user");
    ...
    $sql = "INSERT INTO user(unique_id,nis,fullname,username,password,kelas,alamat,verif,role,join_date)
            VALUES('" . $Anggota . "','" . $nis . "','" . $fullname . "','" . $username1 . "','" . $password . "','" . $kelas . "','" . $alamat . "','" . $verif . "','" . $role . "','" . $join_date . "')";
    $sql .= mysqli_query($koneksi, $sql);

    if (isset($_FILES['img']) && $_FILES['img']['tmp_name'] != '') {
        $funame = strtotime(date('y-m-d H:i')) . '_' . $_FILES['img']['name'];
        $move = move_uploaded_file($_FILES['img']['tmp_name'], 'assets/dist/img' . $fname);
        $data .= ", img = '$fname' ";
    }
